...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, Events, ComponentType, ModalBuilder, TextInputBuilder, TextInputStyle } from 'discord.js'
...
export const connectBOT = async () => {
  ...
  client.on(Events.InteractionCreate, async (interaction: any) => {
    ...
    if (interaction.commandName === 'post-announcements') {
      ...
      await interaction.reply('@everyone Collab Announcements will be posted in the channel');
    }
  ...
  client.on(Events.GuildMemberAdd, async (member: any) => {
    ...
    await assignRolebyuserid({ userId: user.id, serverId: guild.id, mber: member })
  })
...
export const MakeFakeData = async () => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    await Customer.findOneAndUpdate({ userId: row.userId, collabId: row.collabId }, row, { upsert: true, new: true })
  }
...
export const setPicker = async (params: any) => {
  ...
  const up = await Customer.findOneAndUpdate({ userId: data.userId, collabId: data.collabId }, data, { new: true, upsert: true })
...
export const getConverForm = ({ index, show, particis }: any) => {
  ...
  for (let i in data) {
    if (show == "hide") {
      dstr += ((Number(index) * 10) + Number(i) + 1) + "." + "<@" + data[i].userId + ">(1 entry)"
    } else {
      dstr += ((Number(index) * 10) + Number(i) + 1) + "." + data[i].username + "#" + data[i].discriminator + "(1 entry)"
    }
    dstr += "\n"
  }
  return dstr
  ...
  if (btnstr && btnstr.split(":").length >= 2) {
    ...
    switch (handlebtn) {
      case "picker":
        ...
        const wdata = await setWhitelist({ collabid, index: pickers, ended: false, pickerbtn: false, projectid })
        await interaction.update(wdata)
        break;
      case "partici":
        const particis = await participantData({ collabId: collabid, handle: "start" })
        await interaction.reply(particis)
        break;
      case "next":
      case "prev":
        const particis1 = await participantData({ collabId: collabid, handle: handlebtn })
        await interaction.update(particis1)
        break;
      case "showusermentions":
        const particis2 = await participantData({ collabId: collabid, handle: "cmode" })
...
export const ModalCallback = async (interaction: any) => {
  ...
  if (btnstr && btnstr.split(":").length >= 2) {
    ...
    if (handlebtn == "gotopagemodal") {
      ...
      const pagenumber = interaction.fields.getTextInputValue('pagenumberinput');
      const particis2 = await participantData({ collabId: collabid, handle: "spercialpage", pagenumber })
      if (particis2) {
        await interaction.update(particis2)
      } else {
...
export const participantData = async ({ collabId, handle, pagenumber }: any) => {
  ...
  if (handle == "start") {
    ...
    await CollabPagenation.findOneAndUpdate({ collabId: collabId }, { currentpage, collabId, mode }, { upsert: true, new: true })
    ...
    await CollabPagenation.findOneAndUpdate({ collabId: params.collabId }, { mode: mode })
  ...
  await CollabPagenation.findOneAndUpdate({ collabId: collabId }, { currentpage })

  data = await Customer.find({ collabId }).skip(currentpage - 1).limit(10)

  const particidata = getParticipants({
    collabid: collabId,
    show: mode, totalpage: totalPages, count: totalcount, currentpage: currentpage, prevbutton: prevbutton, nextbutton: nextbutton,
    formdata: getConverForm({ index: currentpage - 1, show: mode, particis: data })
  })

  return particidata
}

export const getParticipants = ({ collabid, show, totalpage, count, currentpage, prevbutton, nextbutton, formdata }: any) => {
  ...
  return {
    content: '', ephemeral: true, embeds: [embed], components: [row, row1]
  };
}
...
export const setWhitelist = async ({ index, collabid, ended, winnersstr, pickerbtn, projectid }: any) => {
  ...
  const { expiretime, expiretimemin } = ConverToTime((init.valueOf() - new Date().valueOf()) / 1000 / 60)
  if (expiretime < 0) {
    ended = true
  }
  if (ended) {
...
export const showGotopgaeModal = async (interaction: any, collabid: any) => {
  ...
  const pagenumberActionRow = new ActionRowBuilder().addComponents(PagenumberInput);
...
export const getWinners = async ({ collabid, winners }: any) => {
  ...
  const totalcount = await Customer.countDocuments({ collabId: ObjectId(collabid) })
  let wins: any = []
  if (totalcount > 0) {
...
export const FetchEndcollab = async () => {
  ...
  for (let i in endcollabs) {
    ...
    const pjitem: any = await Projects.findOne({ serverId: item.requestBy.projectName })
    ...
    // }
    const { winnersstr, winners } = await getWinners({ collabid, winners: item.openedSpots })
    ...
    if (winnersstr) {
      ...
      const wdata = await setWhitelist({ collabid, index: pickers, ended: true, winnersstr, pickerbtn: true, projectid: projectid })
      const channel = client.channels.cache.get(channelId) as TextChannel;
      if (channel) {
        if (item.messageId) {
...
export const FetchCountDownCollab = async () => {
  ...
  for (let i in endcollabs) {
    ...
    // }
    const pickers = await getPicker(collabid)
    const wdata = await setWhitelist({ collabid, index: pickers, ended: false, pickerbtn: false, projectid: projectid })
    const channel = client.channels.cache.get(channelId) as TextChannel;
    if (channel) {
      if (item.messageId) {
        channel.messages.fetch(item.messageId).then(msg => msg.edit(wdata))
